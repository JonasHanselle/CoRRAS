...
def create_latex(df : pd.DataFrame, decimal_format="{:10.3f}"):
    ...
    for index, row in df.iterrows():
        result += row[0] + " & " + " & ".join(["\\textbf{" + decimal_format.format(x) + "}" if x == row[1:].values.min() else str(x) for x in row[1:]]) + " \\\\ \n"
    result += "\\toprule \n"
    result += "\\end{tabular} \n"
    print(result)
...
for scenario_name in scenario_names:
    ...
    comparison_data.append([
        scenario_name,
        df_baseline_rf["run_status"].value_counts(normalize=True)["ok"],
        df_baseline_lr["run_status"].value_counts(normalize=True)["ok"],
        df_corras_linear["run_status"].value_counts(normalize=True)["ok"],
        df_corras_quadratic["run_status"].value_counts(normalize=True)["ok"],
        df_corras_nnh["run_status"].value_counts(normalize=True)["ok"]
    ])
comparison_frame = pd.DataFrame(
    data=comparison_data,
    columns=["Scenario", "RF", "LR", "PL-Lin", "PL-Quad", "Hinge-NN"])
