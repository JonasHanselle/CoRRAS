...
for i_split in range(1, num_splits+1):
    ...
    for index, row in test_scenario.feature_data.iterrows():
        ...
        result_data_lr.append([i_split, index, *predicted_performances])

performance_cols = [x + "_performance" for x in scenario.performance_data.columns]

result_columns_lr = ["split", "problem_instance"]
result_columns_lr += performance_cols
results_lr = pd.DataFrame(data=result_data_lr, columns=result_columns_lr)
results_lr.to_csv(result_path+"lr-"+scenario.scenario+".csv", index_label="id")