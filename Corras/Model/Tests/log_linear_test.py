...
class TestLogLinearModel(unittest.TestCase):
    ...
    def test_nll(self):
        ...
        features_train = np.random.randint(low=0, high=30, size=(100,2))
        features_test =  [[3,8], [8,4], [2,6]]
        def create_ranking(feature_list):
            ...
            for features in feature_list:
                ...
                # elif features[0] < features[1] and features[1] < features[2]:
                #     print("case 2")
                #     rankings.append([1,2,3])
                else:
                    print("case 2")
        ...
        train_inst = pd.DataFrame(data=features_train,columns=["a","b"])
        test_inst = pd.DataFrame(data=features_test,columns=["a","b"])
