...
use_max_inverse_transform_values = ["max_par10"]
...
for scenario_name, lambda_value, split, seed, use_quadratic_transform, use_max_inverse_transform, scale_target_to_unit_interval in shard:
    ...
    if not exists:
        ...
        if use_quadratic_transform:
            ...
            train_features = pd.DataFrame(data=quad_data,index=train_features.index,columns=new_cols)
        ...
        train_performances = pd.DataFrame(data=perf,index=train_performances.index,columns=train_performances.columns)
        print(order)
        inst, perf, rank = util.construct_numpy_representation_with_pairs_of_rankings(
        ...
        for index, row in test_scenario.feature_data.iterrows():
            ...
            if use_quadratic_transform:
                row_values = polytransform.transform(row_values.reshape(1,-1))
        ...
        print("File already exists, skipping the parameter configuraiton " + params_string +"!")
    # except Exception as exc:
        # print("Something went wrong during computation with parameters " + params_string + " message: " + str(exc))